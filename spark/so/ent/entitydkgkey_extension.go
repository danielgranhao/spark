...
// GetEntityDkgKey fetches the entity DKG key with its signing keyshare.
...
func GetEntityDkgKey(ctx context.Context, db *Client) (*EntityDkgKey, error) {
	...
	_, err = entityDkgKey.Edges.SigningKeyshareOrErr()
	if err != nil {
...
func GetEntityDkgKeyPublicKey(ctx context.Context, db *Client) (keys.Public, error) {
	entityDkgKey, err := GetEntityDkgKey(ctx, db)
	if err != nil {
		return keys.Public{}, err
	}
	return entityDkgKey.Edges.SigningKeyshare.PublicKey, nil
}
