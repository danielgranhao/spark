...
type TreeNode struct {
	...
	// Bitcoin value in satoshis for this node.
	Value uint64 `json:"value,omitempty"`
	// Bitcoin network this node belongs to (Mainnet, Testnet, Regtest).
	Network btcnetwork.Network `json:"network,omitempty"`
	// Current state of the node (e.g., AVAILABLE, TRANSFER_LOCKED, SPLITTED, ON_CHAIN).
	Status schematype.TreeNodeStatus `json:"status,omitempty"`
	// Aggregated public key (user + SE) that controls spending of this node.
	VerifyingPubkey keys.Public `json:"verifying_pubkey,omitempty"`
	// User's identity public key that owns this node.
	OwnerIdentityPubkey keys.Public `json:"owner_identity_pubkey,omitempty"`
	// User's signing public key derived from their identity key for this specific node.
	OwnerSigningPubkey keys.Public `json:"owner_signing_pubkey,omitempty"`
	// Output index of this node in the parent transaction.
	Vout int16 `json:"vout,omitempty"`
	// Block height when this node's transaction was confirmed on-chain, value is set to nil if the node transaction is not confirmed on-chain.
	NodeConfirmationHeight uint64 `json:"node_confirmation_height,omitempty"`
	// Block height when the refund transaction was confirmed on-chain, value is set to nil if the refund transaction is not confirmed on-chain.
	...
	// A transaction that the watctower can broadcast on behalf of the user which spends the node tx and pays to the user.
	...
	// A transaction that the watctower can broadcast on behalf of the user which spends the direct tx and pays to the user.
	...
	RawRefundTxid schematype.TxID `json:"raw_refund_txid,omitempty"`
	// Valid transaction ID of the direct refund transaction. Generated by hooks from direct_refund_tx.
	DirectRefundTxid schematype.TxID `json:"direct_refund_txid,omitempty"`
